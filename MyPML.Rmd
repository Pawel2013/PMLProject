...
colnamesTrain <- colnames(train)
colnamesTest <- colnames(test)
# find the matching names
commonNames <- matchingNames <- names(train)[names(train) %in% names(test)]
# select only common names in training set
trainNew <- train[,(colnamesTrain %in% commonNames)]
# select only common names in test set
testNew <- test[,(colnamesTest %in% commonNames)]

# add column "classe
trainNew["classe"] <- NA
trainNew$classe <- train$classe
# set the name of last column
#colnames(trainNew)[dim(trainNew)[2]] <- "classe"

# For Cross Validation
set.seed(1234)
inTrain <- createDataPartition(y = trainNew$classe, p = 0.8, list = F)
trainingCV <- trainNew[inTrain,]
# validation test
validatingCV <- trainNew[-inTrain,]
